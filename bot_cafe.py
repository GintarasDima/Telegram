...
def get_keyboard(keyboard_name):
    with open('menu.json', encoding='UTF-8') as menu_config:
        config_data = json.load(menu_config)
    actual_keyboard = list(filter(lambda el: el['keybord_name'] == keyboard_name, config_data))[0]
...
def start(message):
    bot.send_message(message.chat.id, f'Привет{message.from_user.full_name}', reply_markup=get_keyboard('main'))
